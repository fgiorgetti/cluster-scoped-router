...
`skupper-router-multi-tenant` DaemonSet; if multiple are found, a selection menu is
presented.
...
| Prompt | Description |
|---|---|
| Router namespace | Shown only when multiple `skupper-router-multi-tenant` DaemonSets are found |
...
`skupper-router-multi-tenant` DaemonSet; if multiple are found, a plain terminal `select`
prompt is shown.
